...
impl From<ReadCharError> for io::Error {
    fn from(e: ReadCharError) -> io::Error {
        e.into_io_error()
    }
}
...
pub trait BufReadCharsExt: BufRead {
    ...
    /// [`io::Result`]`<char>`.
    fn chars(&'_ mut self) -> Chars<'_, Self> {
        Chars(self)
    }
    ...
    fn chars_raw(&'_ mut self) -> CharsRaw<'_, Self> {
        CharsRaw(self)
    }
    ...
    fn read_char_raw(&mut self) -> Result<Option<char>, ReadCharError> {
        match read_byte_and_ignore_interrupts(self) {
            ...
            Ok(Some(lead_byte)) => {
                ...
                if leading_ones == 0 {
                    return Ok(Some(char::from(lead_byte)));
                }
                ...
                let mut item = ((lead_byte & LEAD_BYTE_MASK[tail_bytes_count as usize]) as u32)
                ...
                for tail_byte_index in (0..tail_bytes_count).rev() {
                    match read_byte_and_ignore_interrupts(self) {
                        ...
                        Ok(Some(tail_byte)) => {
                            ...
                            item |= ((tail_byte & TAIL_BYTE_MASK) as u32)
...
mod tests {
    ...
    fn edgecase_three_four_bytes() {
        ...
        assert_eq!(
            vec!['\u{010000}'],
            BufReader::new(&[0xF0, 0x90, 0x80, 0x80][..])
                .chars_raw()
                .map(|x| x.unwrap())
                .collect::<Vec<_>>()
        );
    ...
    fn edgecase_four_bytes_max() {
        assert_eq!(
            vec!['\u{10FFFF}'],
            BufReader::new(&[0xF4, 0x8F, 0xBF, 0xBF][..])
                .chars_raw()
                .map(|x| x.unwrap())
                .collect::<Vec<_>>()
        );
    ...
    fn read_invalid_sequences() {
        let mut bytes = BufReader::new(
            &[
                0x81, 0x82, 0xC1, 0x07, 0xC1, 0x87, 0xC2, 0xC2, 0x82, 0xF7, 0x88, 0x89, 0x07,
            ][..],
        );
        let res = bytes.chars_raw().collect::<Vec<_>>();
        assert_eq!(9, res.len());
        ...
        assert_eq!(
            &[0xF7, 0x88, 0x89][..],
            res[7].as_ref().err().unwrap().as_bytes()
        );
    ...
    fn read_array(b: Vec<u8>) -> bool {
        ...
        BufReader::new(&b[..]).chars_raw().for_each(|c| {
            t.append(&mut c.map_or_else(
                |e| e.as_bytes().to_vec(),
                |s| s.to_string().as_bytes().to_vec(),
            ))
        });
        b == t
    }
